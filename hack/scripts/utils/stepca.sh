...
STEP_CLI_IMAGE="${STEP_CLI_IMAGE:-smallstep/step-cli:latest}"
...
stepca_apply_bundle() {
  ...
  kubectl --kubeconfig "$kubeconfig" -n "$namespace" create secret generic "${app_name}-files" \
    ...
    --from-file=provisioner_password="$provisioner_password_file" \
    --dry-run=client -o yaml | kubectl --kubeconfig "$kubeconfig" apply -f -

  manifest_template="$POC_DIR/manifests/step-ca.yaml"
