...
source "$SCRIPT_DIR/../helpers/kube/kubeconfig.sh"
source "$SCRIPT_DIR/../helpers/kube/wait.sh"
source "$SCRIPT_DIR/../helpers/kube/cni.sh"
source "$SCRIPT_DIR/../helpers/render/manifests.sh"
...
manifest_path="$OUT_DIR/workload/manifests/${MODE}-cluster.yaml"
initial_control_plane_replicas="$(initial_control_plane_replicas_for_mode "$MODE")"
target_control_plane_replicas="$(control_plane_replicas_for_mode "$MODE")"
...
ensure_out_dirs
mkdir -p "$out_dir"
render_mode_manifest "$MODE" "$manifest_path" "$namespace"
