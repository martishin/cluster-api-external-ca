...
if [[ "$MODE" == "external-ca" ]]; then
  node_sign_script="$POC_DIR/assets/signer/capi-node-sign.sh"
  worker_sign_script="$POC_DIR/assets/signer/capi-worker-sign.sh"
  signer_config_file="$POC_DIR/assets/signer/config.env"
  if [[ ! -s "$node_sign_script" || ! -s "$worker_sign_script" || ! -s "$signer_config_file" ]]; then
    echo "missing signer scripts under $POC_DIR/assets/signer" >&2
    exit 1
  fi
  ...
  kubectl -n "$cluster_namespace" create secret generic "${cluster_name}-step-ca-signer" \
    ...
    --from-file=provisioner-password="$provisioner_password_file" \
    --from-file=signer-config="$signer_config_file" \
    --from-file=root-ca.crt="$workload_bundle/certs/root_ca.crt" \
    --from-file=script="$node_sign_script" \
    --from-file=worker-script="$worker_sign_script" \
